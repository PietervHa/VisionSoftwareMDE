...
Usage:
    python QC_tools/migrate_jsonl_to_db.py
    python QC_tools/migrate_jsonl_to_db.py --dry-run
    python QC_tools/migrate_jsonl_to_db.py --force   # re-run even if the table already has rows
...
# (running it directly puts only QC_tools/ on sys.path, not the repo root).
